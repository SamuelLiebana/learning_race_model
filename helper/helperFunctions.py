...
def make_invSigma(hyper, days, missing_trials, N, K):
    """Returns the banded prior matrix for \eta
    ...
    # this is if you use the same sigma for all parameters
    if np.isscalar(sigma):
        invSigma_flat_k = np.ones(N) * sigma**2  # populate diagonal with sigmas
...
def DT_X_D(ddlogprior, K):
    """
    Computes D.T @ X @ D, where D is the block
